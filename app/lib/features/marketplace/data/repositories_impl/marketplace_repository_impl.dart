...
Map<String, dynamic> _draftToPayload(ListingDraft draft) => {
      ...
      if (draft.suggestedMarketPrice != null) 'suggestedMarketPrice': draft.suggestedMarketPrice,
...
class MarketplaceRepositoryImpl implements MarketplaceRepository {
  ...
  Future<Either<Failure, List<CategoryEntity>>> getCategories() =>
      _guard(() async => (await _remote.getCategories()).map((c) => c.toEntity()).toList());
  ...
  Future<Either<Failure, CropListingEntity>> createListing(ListingDraft draft) =>
      _guard(() async => (await _remote.createListing(_draftToPayload(draft))).toEntity());

  @override
  Future<Either<Failure, CropListingEntity>> updateListing(String id, ListingDraft draft) =>
      _guard(() async => (await _remote.updateListing(id, _draftToPayload(draft))).toEntity());
  ...
  @override
  Future<Either<Failure, bool>> toggleBookmark(String listingId) => _guard(() => _remote.toggleBookmark(listingId));

  @override
  Future<Either<Failure, List<CropListingEntity>>> getBookmarks() =>
      _guard(() async => (await _remote.getBookmarks()).map((m) => m.toEntity()).toList());
}
